...
class AnalysisController extends Controller
{
    ...
    public function performAnalysis(Request $request)
    {
        ...
        if ($activities->isEmpty()) {
            return response()->json([
                "analysis" =>
                    "Tidak ada aktivitas yang ditemukan pada rentang tanggal yang dipilih.",
            ]);
        }
        ...
        try {
            ...
            if ($response->successful()) {
                return $response->json(
                    ...
                    "Maaf, analisis tidak dapat dibuat saat ini.",
                );
            }
    ...
    private function buildAnalysisPrompt(
        ...
        $systemPrompt = file_get_contents(
            resource_path("prompts/analysis_prompt.txt"),
        );
        return $systemPrompt .
    ...
    private function buildSingleActivityPrompt(
        Activity $activity,
        array $preparedData,
    ): string {
        ...
        $systemPrompt = file_get_contents(
            resource_path("prompts/single_activity_analysis_prompt.txt"),
        );
    ...
    {
        $streams = is_string($activity->streams)
            ? json_decode($activity->streams, true)
            : $activity->streams;

        if (empty($streams) || !is_array($streams)) {
            return [
                "error" => "No stream data available for detailed analysis.",
            ];
        }
        ...
        $avgHr = !empty($validHr)
            ? round(array_sum($validHr) / count($validHr))
            : null;
        $avgWatts = !empty($validWatts)
            ? round(array_sum($validWatts) / count($validWatts))
            : null;
